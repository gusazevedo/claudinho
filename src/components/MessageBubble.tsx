...
export function MessageBubble({ message }: { message: Message }) {
  ...
  return (
    <div className={`flex animate-slide-up ${isUser ? 'justify-end' : 'justify-start'}`}>
      {!isUser && (
        <div className="w-7 h-7 rounded-full bg-accent flex items-center justify-center shrink-0 mt-0.5 mr-3">
      ...
      <div className={`max-w-[72%] ${isUser ? '' : 'flex-1'}`}>
        ...
        <p className="text-text-muted text-[10px] mt-1 px-1">{formatTime(message.timestamp)}</p>
      </div>

      {isUser && (
        <div className="w-7 h-7 rounded-full bg-bg-elevated border border-border flex items-center justify-center shrink-0 mt-0.5 ml-3">
